...
void Sh2Logger::parseArgs(int argc, const char* argv[]) {
    ...
    TCLAP::ValueArg<int>
            clearDcdArg("",
                        ...
                        "configuration file if provided, otherwise defaults to 0 (do not clear).",
                        false,
                        0,
                        &zero_one_constraint);
    cmd.add(clearDcdArg);
    ...
    // --wheel_source [-|file]
    TCLAP::ValueArg<std::string> wheelSourceArg("w",
                                                "wheel_source",
    ...
    static const json templateContents = {
            ...
            {"orientation", "ned"},
            {
                    "sensorList",
                    {
                            ...
                            {"Raw Optical Flow",
                             {{"rate", 0}, {"sensorSpecific", 0}, {"sniffEnabled", 0}}}, // 0x2C
...
bool ParseJsonBatchFile(std::string inFilename, LoggerApp::appConfig_s* pAppConfig) {
    ...
    for (json::iterator it = jBat.begin(); it != jBat.end(); ++it) {
        ...
        if (it.key().compare("calEnable") == 0) {
            ...
            std::cout << "INFO: (json) Calibration Enable : "
                      << static_cast<uint32_t>(pAppConfig->calEnableMask) << "\n";
            ...
            for (json::iterator sl = it.value().begin(); sl != it.value().end(); ++sl) {
                ...
                if (config.sensorId <= SH2_MAX_SENSOR_ID && config.reportInterval_us > 0) {
                    std::cout << "INFO: (json)      Sensor ID : "
                              << static_cast<uint32_t>(config.sensorId);
                    std::cout << " - " << sl.key();
